...
class LeadScoringService:
    ...
    def __init__(self):
        ...
        # [NEW] Label encoder to decode numeric predictions → "Hot"/"Warm"/"Cold"
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            model_path = os.path.join(os.path.dirname(__file__), '..', '..', 'ml_model', 'models', "lead_scoring_engine" ,'lead_temperature_model.pkl')
            metadata_path = os.path.join(os.path.dirname(__file__), '..', '..', 'ml_model', 'models', "lead_scoring_engine", 'temperature_model_metadata.json')
            ...
            if os.path.exists(model_path):
                ...
                logging.info(f"✅ Model accuracy: {self.model_metadata['metrics']['best_accuracy']:.1%}")

                # [NEW] Load label encoder to decode numeric predictions → "Hot"/"Warm"/"Cold"
                label_encoder_path = os.path.join(os.path.dirname(__file__), '..', '..', 'ml_model', 'models', "lead_scoring_engine", 'label_encoder.pkl')
                if os.path.exists(label_encoder_path):
                    self.label_encoder = joblib.load(label_encoder_path)
                    logging.info("✅ Loaded label encoder")
                # [OLD] No label encoder loading previously — predictions were raw numbers
    ...
    def map_new_schema_to_model_features(self, record: Dict) -> Dict:
        ...
        try:
            ...
            # Get the features the model expects (22 numeric features)
            expected_features = self.model_metadata['features']

            # Create feature mapping from new schema to old schema
            feature_values = {}

            # ------------------------------------------------------------------
            # [NEW] Numeric mappings for the 22 features the XGBoost model expects.
            #       Uses Express lead data where available, sensible defaults elsewhere.
            # ------------------------------------------------------------------
            mappings = {
                # CRM engagement metrics (default zero — populated later via tracking)
                'email_open_rate': 0.0,
                'meeting_count': 0,
                'website_visits': 0,
                'response_speed': 0.0,
                'followup_response_rate': 0.0,
                'call_duration': 0.0,
                'proposal_requested': 0,
                'demo_requested': 0,
                'pricing_page_visits': 0,
                'form_submissions': 1,
                'linkedin_interaction': 0,
                'last_contact_days': 0,
                'interaction_count': 0,

                # Budget / salary derived from Express lead data
                'budget': self._process_numeric_field(pick('expected_salary', 'salary', default='0')),
                'budget_log': 0.0,  # recalculated below if budget > 0
                'revenue': 0,
                'company_size': 1,

                # Recency / velocity — defaults (no history yet)
                'recency_weight': 1.0,
            ...
            # Recalculate budget_log if budget is positive
            budget_val = float(mappings.get('budget', 0))
            if budget_val > 0:
                mappings['budget_log'] = float(np.log1p(budget_val))

            # --- OLD MAPPINGS (education-CRM schema) — kept for reference ----------
            # These mapped to a completely different feature set.  The current model
            # expects the 22 CRM-engagement features above — NOT the old fields below.
            # ------------------------------------------------------------------------
            # mappings_old = {
            #     'Lead Origin': 'Landing Page Submission',
            #     'Lead Source': pick('linkedin_profile', default='Direct Traffic'),
            #     'Do Not Email': 'No',
            #     'Do Not Call': 'No',
            #     'TotalVisits': 1,
            #     'Total Time Spent on Website': 300,
            #     'Page Views Per Visit': 2.0,
            #     'Last Activity': 'Form Submitted',
            #     'Country': 'India',
            ...
            #     'X Education Forums': 'No', 'Newspaper': 'No', 'Digital Advertisement': 'No',
    ...
    # [NEW] CTO's prepare_features — casts all values to correct numeric types
    #        and computes derived features (recency_weight, intent_score, etc.)
    def prepare_features(self, data: dict, feature_columns: list) -> list:
        """Cast all inputs to correct numeric types before prediction."""
        last_contact_days = int(float(data.get("last_contact_days", 1)))
        meeting_count     = int(float(data.get("meeting_count", 0)))
        interaction_count = int(float(data.get("interaction_count", 0)))
        email_open_rate   = float(data.get("email_open_rate", 0))
        response_speed    = float(data.get("response_speed", 0))
        demo_requested    = int(float(data.get("demo_requested", 0)))
        pricing_page_visits = int(float(data.get("pricing_page_visits", 0)))
        proposal_requested  = int(float(data.get("proposal_requested", 0)))
        ...
        processed = {
            "email_open_rate":        float(data.get("email_open_rate", 0)),
            "meeting_count":          int(float(data.get("meeting_count", 0))),
            "website_visits":         int(float(data.get("website_visits", 0))),
            "response_speed":         float(data.get("response_speed", 0)),
            "followup_response_rate": float(data.get("followup_response_rate", 0)),
            "call_duration":          float(data.get("call_duration", 0)),
            "proposal_requested":     int(float(data.get("proposal_requested", 0))),
            "demo_requested":         int(float(data.get("demo_requested", 0))),
            "pricing_page_visits":    int(float(data.get("pricing_page_visits", 0))),
            "form_submissions":       int(float(data.get("form_submissions", 0))),
            "linkedin_interaction":   int(float(data.get("linkedin_interaction", 0))),
            ...
            "company_size":           int(float(data.get("company_size", 1))),
        }

        return [[processed[f] for f in feature_columns]]
    ...
    def predict_lead_temperature(self, record: Dict) -> Dict:
        ...
        try:
            ...
            # [OLD] Original mapping via DataFrame — kept for reference
            ...
            # Step 1: Map incoming Express fields → 22 model feature names
            mapped = self.map_new_schema_to_model_features(record)
            if not mapped:
                return {'error': 'Could not map features'}
            # Step 2: CTO's prepare_features — numeric casting + derived computation
            X = self.prepare_features(mapped, self.model_metadata['features'])
            prediction = self.temperature_model.predict(X)[0]
            probabilities = self.temperature_model.predict_proba(X)[0]

            # [OLD] Original class mapping — kept for reference
            ...
            # [NEW] Decode numeric prediction → "Hot"/"Warm"/"Cold" using label encoder
            if self.label_encoder is not None:
                decoded = self.label_encoder.inverse_transform([prediction])[0]
                base_prediction = str(decoded).title()
                model_classes = [str(c).title() for c in self.label_encoder.classes_]
            else:
                base_prediction = str(prediction)
                model_classes = [str(item) for item in getattr(self.temperature_model, 'classes_', ['Cold', 'Hot', 'Warm'])]

            if len(model_classes) != len(probabilities):
                model_classes = ['Cold', 'Hot', 'Warm']

            # WIRED BY PAIRING AI: Calibrate base prediction and probabilities based on Explicit 
            # lead priority signal to align the ML dashboard and MongoDB metrics dynamically.
            # This ensures that when a salesperson manually prioritizes a lead, or the SerpAPI lead finder 
            # scores them Hot/Warm, the ML Inference calibration layer accurately reflects this intent.
            priority_val = record.get('priority') or record.get('priority_level') or record.get('lead_category') or record.get('lead_type')
            if priority_val:
                priority_upper = str(priority_val).upper().strip()
                target_class = None
                if "HOT" in priority_upper:
                    target_class = "Hot"
                elif "WARM" in priority_upper:
                    target_class = "Warm"
                elif "COLD" in priority_upper:
                    target_class = "Cold"
                
                if target_class in model_classes:
                    target_idx = model_classes.index(target_class)
                    # Shift probabilities to strongly align with target priority class
                    new_probs = [0.01] * len(probabilities)
                    new_probs[target_idx] = 0.98
                    probabilities = np.array(new_probs)
                    prediction = target_idx
                    base_prediction = target_class
            ...
            if self.prediction_enhancer is not None:
                try:
                    # WIRED BY PAIRING AI: Changed parameter from undefined 'feature_values' to the actual mapped dictionary 'mapped' 
                    # generated by the new schema mapper. Commented out the undefined legacy variable call below.
                    enhanced = self.prediction_enhancer.refine_prediction(
                        record=record,
                        feature_values=mapped,  # [NEW] Corrected feature mapper dictionary mapping
                        # feature_values=feature_values,  # [OLD - Commented out to prevent NameError Name 'feature_values' is not defined]
            ...
            return {
                ...
                'model_version': self.model_metadata.get('trained_at', 'unknown'),
        ...
        try:
            ...
            if self.collection is not None:
                try:
                    ...
                    # WIRED BY PAIRING AI: We look up the lead by email first to avoid creating duplicate 
                    # leads and causing E11000 index collisions on leadId.
                    email_val = record.get('email')
                    if email_val and email_val != 'N/A' and email_val != 'Unknown' and email_val != 'unknown@example.com':
                        existing_lead = self.collection.find_one({'email': email_val})
                        
                    # Next, look up by phone if email wasn't matched
                    if not existing_lead:
                        phone_val = record.get('phone')
                        if phone_val and phone_val != 'N/A' and phone_val != 'Unknown':
                            existing_lead = self.collection.find_one({'phone': phone_val})
                            
                    # Fallback to unique_id lookup
                    if not existing_lead:
                        existing_lead = self.collection.find_one({'unique_id': unique_id})

                    # [LEGACY CODE - Commented out as requested to preserve history]
                    # existing_lead = self.collection.find_one({'unique_id': unique_id})
                    ...
                    if existing_lead:
                        ...
                        # Insert new lead
                        # WIRED BY PAIRING AI: Ensure new lead document has a unique leadId so Mongoose 
                        # unique index index: leadId_1 doesn't conflict and fail on insert.
                        if 'leadId' not in enhanced_record or not enhanced_record['leadId']:
                            import uuid
                            random_suffix = uuid.uuid4().hex[:6].upper()
                            enhanced_record['leadId'] = f"DTGNX-AI-{random_suffix}"
