    ...
    try:
        ...
        class MockService:
            def process_lead_with_ml(self, data):
                return {"error": f"ML service unavailable: {error_msg}"}
...
def get_auth_service():
    ...
    if os.getenv('SKIP_MONGODB', 'false').lower() == 'true':
        ...
        from services.auth_service_inmemory import dev_auth_service
        _cached_auth_service = dev_auth_service
        return _cached_auth_service

    try:
        # Try MongoDB auth service
        from services.auth_service import get_auth_service as get_db_auth_service
...
def get_conversion_lead_scoring_modules() -> Dict[str, Any]:
    ...
    service_dir = Path(__file__).resolve().parent
    scoring_module_path = service_dir / "services" / "lead_scoring_service" / "lead_scoring_service.py"
...
def get_ai_insights_service():
    ...
    try:
        ...
        from services import get_ai_insights_service as resolver
        return resolver()
    except Exception as e:
        logging.warning(f"Primary AI insights import failed, attempting fallback path load: {e}")

    service_dir = Path(__file__).resolve().parent
    fallback_path = service_dir / "services" / "smart lead summary" / "ai_insights_service.py"
...
async def enrich_company_data(payload: CompanyEnrichmentRequest):
    ...
    try:
        ...
        from services.serpapi_service import build_search_context, pick_best_website, search_google_business_results
...
async def chatbot_chat(payload: ChatbotChatRequest):
    ...
    try:
        from services.chatbot import handle_chat_request_async
